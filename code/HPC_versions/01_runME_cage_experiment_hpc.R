...
source(here::here("code/run_simulations_cage.R"))
...
idx <- 1#as.numeric(Sys.getenv("SLURM_ARRAY_TASK_ID"))
...
if(isTRUE(MIN_VALUES)){
  param_list$parameter_set <- str_replace(param_list$parameter_set, "_400_", "_420_")  
}
if(isTRUE(MAX_VALUES)){
  param_list$parameter_set <- str_replace(param_list$parameter_set, "_800_", "_420_")  
}



if(!dir.exists(here::here(glue::glue("outputs/cage_simulations/{param_list$parameter_set}")))){
  dir.create(here::here(glue::glue("outputs/cage_simulations/{param_list$parameter_set}")))
}
