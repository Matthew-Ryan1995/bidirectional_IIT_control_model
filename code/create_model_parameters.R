...
param_save_name <- glue::glue("data/param_list_MIN_VALUES_{MIN_VALUES}_MAX_VALUES_{MAX_VALUES}_TRANSFORM_DEATHRATE_{TRANSFORM_DEATHRATE}_TRANSFORM_BIRTHRATE_{TRANSFORM_BIRTHRATE}.Rds")
...
if(isTRUE(MIN_VALUES)){
  genotype_selector <- paste(genotype_selector_0, "_min", sep = "")
  death_selector <- paste(genotype_selector_0, "_max", sep = "") # The larger death rate (smaller life span) goes with lower parameters and long birth rate: This makes small population
}else if(isTRUE(MAX_VALUES)){
  genotype_selector <- paste(genotype_selector_0, "_max", sep = "")
  death_selector <- paste(genotype_selector_0, "_min", sep = "") # The smaller death rate (long life) goes with larger parameters: This makes large population
}else{
...
populate_m_values <- function(k, numMaleClasses, phi, maleDeathRate, M1, M_bar, transitionRate_m){
  ...
  if(k < numMaleClasses[1]){
    ...
    return(round(transitionRate_m*phi[1]^(k-2)/maleDeathRate[1] * M1[1]))
  }
}
...
for (k in seq(numMaleClasses[1])){
  state_vector[sprintf("%s_wAlbAB_m_%d", populationNames, k)] <- populate_m_values(k, numMaleClasses, phi, maleDeathRate, M1, M_bar, transitionRate_m)
...
parameter_set <- glue::glue(
  "numMaleClasses_{numMaleClasses[1]}_numFemaleClasses_{numFemaleClasses[1]}_numImmatureClasses_{numImmatureClasses[1]}_carryingCapacityByPopulation_vector_{carryingCapacityByPopulation_vector[1]}_maleDeathRate_{maleDeathRate[1]}_femaleDeathRate_{femaleDeathRate[1]}_propMated_{propMated[1]}_femaleBirthRate_{femaleBirthRate[1]}"
)
