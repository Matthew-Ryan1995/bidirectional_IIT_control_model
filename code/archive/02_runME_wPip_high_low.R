...
## This code runs simulations for wAlbAB having baseline parameters, but we vary the following for wPip:
### Birth rate
### Death rate
### Mating rate
...
source(here::here("code/run_simulations_vary_wPip.R"))
...
param_load_name <- glue::glue("data/param_list_wPip_{wpip_level}_TRANSFORM_DEATHRATE_{TRANSFORM_DEATHRATE}_TRANSFORM_BIRTHRATE_{TRANSFORM_BIRTHRATE}.Rds")
...
if(!dir.exists(here::here(glue::glue("outputs/intervention_simulations_vary_wPip/{param_list$parameter_set}")))){
  dir.create(here::here(glue::glue("outputs/intervention_simulations_vary_wPip/{param_list$parameter_set}")))
}
...
for(idx in 1:nrow(scenario_parameters)){
  ...
  res <- run_simulations_vary_wPip(OF = OF, FCP = FCP, female_immigration_rate = female_immigration_rate,
