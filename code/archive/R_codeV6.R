...
# Are we applying population limits anywhere? 
# I could not see carrying limits being taken into account
# Notes:
## Carrying capacity not working
## femaleMatingRateByPopulation wrong, missing Mbar
## Immatures do not count towards the carrying capacity
## Is the issue occuring because of the addition of female immature classes?
## Does this make it so that all the Ibars are wrong
## Why is this a vector of ones: transitionRateByName_vector
## Female birth rate: Is this the birth rate of female eggs, or just the birth rate?
## I suspect that femaleMatingRateByPopulation and M_bar are wrong
## Can males die in each class? This changes things drastically
## It is not properly counting all immature classes.
## Currently sex of mozzie is determined by a coin flip when constructing the possible transitions
### This should be fine, given how short the time steps, but can this possible make it less 
### likely that a particular sex is born?
## Is death rate the natural life span of the mozzie? If so, death should not be possible 
### at each age group.  Removing this really slows down the code (states get bigger)

## Summary thoughts
### No carrying capacity enforced on adult mozzies - This is intended
### Carrying capacity input does not match capacity for immatures (Imax)
### wPip and AlbAB can't produce offspring (CI index) - This is intended, suppression not replacement
### Each genotype only births into their own genotype  - This is intended
### As only males are introduced,  wPip cannot take over (cannot create infants) - This is intended
### I have fixed some of the transition problems, but there are possibly more
### Adding the extra male compartments likely alter the derivation of the transition rates
...
# To play
## I don't think the population carrying capacities are working
...
#problem: The below line does not agree with the supplement from \pan.  Missing Mbar. This makes a massive difference
...
femaleMatingRateByPopulation <- (transitionRate_imm * I_bar / 2 - femaleDeathRate * F_bar)/(F_bar * M_bar)
# femaleMatingRateByPopulation <- ((transitionRate_imm / 2) *( (I_max - I_total)/I_max) * F_bar_mated - femaleDeathRate * F_bar) / (M_bar * F_bar * numMaleClasses)
...
state_vector["block01_wAlbAB_f_1_Unmated_None"  ] <- as.numeric(round(F_bar)) #this is set to be the wildtype population of unmated females in the first age class
state_vector["block01_wAlbAB_f_1_wAlbAB_1"  ] <- as.numeric(round(F_bar_mated)) #this is set to be the wildtype population of mated females (by wildtype males) in the first age class
...
# for(i in 1:length(ts))
# {
#   plot(ts[[i]]/div[1], xlab = "Days", ylab = "Population", main = names(ts)[[i]])  
#   lines(ts[[i]]/div[1])  
# }
