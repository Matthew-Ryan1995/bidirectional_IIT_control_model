...
dat_full <- read_rds(here::here("data/intervention_results_vary_wPip.Rds"))
...
for(pp in unique(dat_full$params)){
  ...
  #   TRUE ~ 420
  # )
  cc <- 420 # Fixed, only varying wPip params
  ...
  ggsave(here::here(glue::glue("img/interventions_vary_wPip_time_until_stop_{pp}.png")), plot = p_stop,
  ...
  p_props <- dat_endtime %>% 
    ...
    labs(x = "Immigration rate", y = "Number of simulations with establishment",
         fill="Intervention type", caption="Dotted line = 50% of simulation",
         title = str_c("Parameter set: ", pp, "\n", lifespan)) +
    harrypotter::scale_fill_hp_d("ravenclaw", direction = -1) +
    theme_bw() + 
    theme(legend.position = "bottom")
  
  ggsave(here::here(glue::glue("img/interventions_vary_wPip_prop_establishment_{pp}.png")), plot = p_props,
         height = height, width = 1.618*height, dpi = 300)
  ...
  p_storm <- plot_data %>% 
    ...
    theme_bw() +
    labs(y="Proportion of steady state", caption="Blue=wAlbAB\nred=wPip",
         title = str_c("Parameter set: ", pp, "\n", lifespan)) +
    theme(text = element_text(size = text_size))
  
  ggsave(here::here(glue::glue("img/interventions_vary_wPip_stormcloud_{pp}.png")), plot = p_storm,
         height = height, width = 1.618*height, dpi = 300)
  ...
  ggsave(here::here(glue::glue("img/interventions_vary_wPip_cost_boxplot_{pp}.png")), plot = p_cost_box,
         height = height, width = 1.618*height, dpi = 300)
  ...
  ggsave(here::here(glue::glue("img/interventions_vary_wPip_cost_stormcloud_{pp}.png")), plot = p_cost_storm,
         height = height, width = 1.618*height, dpi = 300)
