## Matt Ryan
## 09/02/2024
### Collect the data from the intrevention simulations
...
# Get directories
dirs <- list.files(here::here("outputs/intervention_simulations_vary_wPip/"))
dirs <- dirs[-1] # get rid of archive

param_combs <- c("High", "Low")

df <- list()

for( idx in 1:length(dirs) ){
  # List all directories
  dr <- list.dirs(here::here(glue::glue("outputs/intervention_simulations_vary_wPip/{dirs[idx]}/")), 
                  full.names = T)
  dr <- dr[-1]
...
write_rds(df, here::here("data/intervention_results_vary_wPip.Rds"))
