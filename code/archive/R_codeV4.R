...
aggregateStates <- function(stateNames, modelOutputs, populationNames, genotypeNames)
{
    ...
    for(p in 1:length(populationNames))
    {
      for(g in 1:length(genotypeNames))
      {
        ...
        theseInds_imm <- getIndicesForStateQuery(stateNames, populationName = populationNames[p], genotype = genotypeNames[g], sex = "imm", ageClass = NA, mate = NA)
...
getStateNames <- function(n_m, n_f, n_i, genotypeNames, populationNames, numClassesBySexAndGenotype_matrix)
{
  #Each population contains:
                            ...
                            #immatures of different genotypes and with n_i classes for each
...
getTransitionRates <- function(state_vector, numClassesBySexAndGenotype_matrix, matingRateByName_vector, ciByName_vector, offspringGenotypeByName_vector, friedsByName_vector, deathRatesByName_vector, birthRateByName_vector, transitionRateByName_vector, emmigrationRate_matrix, immigrationRate_vector, carryingCapacityByPopulation_vector, ImaxByPopulation_vector)
{
  ...
  #Genotype: this can be used to describe wiltype and wolbachia infected types of individuals
  ...
  #
  #immigrationRate_vector: a named vector containing the actual rates (not percapita rates) at which a new individual should appear in this state from "outside" (i.e. not one of the other populations int he model)
  ...
  if(length(immatureIndices) > 0)
  {
    for(i in 1:length(immatureIndices))
    {
      ...
      if(ageClass == numClassesBySexAndGenotype_matrix[genotype, "imm"])
      {
        ...
        transitionRates_vector[counter] <- 0.5*state_vector[ind]*transitionRateByName_vector[stateNames_vector[ind]]
        transitions[ind, counter] <- -1
...
populationNames <- c("block1") #subpopulations in the metapopulation
genotypeNames <- c("wAlbAB", "wPip") #The different genotypes or wolbachia types
numMaleClasses <- rep(20, length(genotypeNames)) #The number of age classes (days) to use in the model for males of the different genotypes (don't have to be the same for each genotype)
numFemaleClasses <- rep(1, length(genotypeNames)) #The number of age classes (days) to use in the model for females of the different genotypes (don't have to be the same for each genotype)
numImmatureClasses <- rep(10, length(genotypeNames)) #The number of immature classes (days) to use in the model.  These give rise to a lag between birth and emergence as an adult
...
carryingCapacityByPopulation_vector <- rep(420, length(populationNames)) #The number of adult mosquitoes in each of the populations at equilibrium
names(carryingCapacityByPopulation_vector) <- populationNames #Don't edit this line
transitionRate_imm <- 1 #don't edit this line
...
femaleBirthRate <- 0.4 #the birth rate of females
...
I_total <- I_bar*numImmatureClasses #don't edit this line
...
ImaxByPopulation_vector <- rep(I_max, length(populationNames))
...
state_vector["block1_wPip_m_1"] <- 0 #make this whatever you want the population of wPip males (first age class) to be in the population named "block1"
state_vector["block1_wPip_f_1_Unmated_None"  ] <- 0 #make this whatever you want the population of unmated wPip females (first age class) to be in the population named "block1"
state_vector["block1_wAlbAB_m_1"  ] <-  200#as.numeric(round(M_bar)) #this is set to be the wildtype population of males in the first age class
state_vector["block1_wAlbAB_f_1_Unmated_None"  ] <- 40#as.numeric(round(F_bar)) #this is set to be the wildtype population of unmated females in the first age class
state_vector["block1_wAlbAB_f_1_wAlbAB_1"  ] <- 160#as.numeric(round(F_bar_mated)) #this is set to be the wildtype population of mated females (by wildtype males) in the first age class
state_vector["block1_wAlbAB_imm_1"] <- round(I_bar) #this is set to be the wildtype population of immatures in the 1st age class
state_vector["block1_wAlbAB_imm_2"] <- round(I_bar) #this is set to be the wildtype population of immatures in the 2nd age class
state_vector["block1_wAlbAB_imm_3"] <- round(I_bar) #this is set to be the wildtype population of immatures in the 3rd age class
state_vector["block1_wAlbAB_imm_4"] <- round(I_bar) #this is set to be the wildtype population of immatures in the 4th age class
state_vector["block1_wAlbAB_imm_5"] <- round(I_bar) #this is set to be the wildtype population of immatures in the 5th age class
state_vector["block1_wAlbAB_imm_6"] <- round(I_bar) #this is set to be the wildtype population of immatures in the 6th age class
state_vector["block1_wAlbAB_imm_7"] <- round(I_bar) #this is set to be the wildtype population of immatures in the 7th age class
state_vector["block1_wAlbAB_imm_8"] <- round(I_bar) #this is set to be the wildtype population of immatures in the 8th age class
state_vector["block1_wAlbAB_imm_9"] <- round(I_bar) #this is set to be the wildtype population of immatures in the 9th age class
state_vector["block1_wAlbAB_imm_10"] <- round(I_bar) #this is set to be the wildtype population of immatures in the 10th age class
...
for(i in 1:numMaleClasses["wAlbAB"])
{
  ...
  {
      immigrationRate_vector[ind_m_wAlb_i] <- overallMaleImmigrationRate*(pexp((1/transitionRate_m)*i, rate = maleDeathRate) - pexp((1/transitionRate_m)*(i - 1), rate = maleDeathRate))
  }
  else
  {
    immigrationRate_vector[ind_m_wAlb_i] <- overallMaleImmigrationRate*(1 - pexp((1/transitionRate_m)*i, rate = maleDeathRate))
  }
...
for(i in 1:numFemaleClasses["wAlbAB"])
{
  ...
  else
  {
    ...
    {
      immigrationRate_vector[ind_f_wAlb_i] <- overallFemaleImmigrationRate*(pexp((1/transitionRate_f)*i, rate = femaleDeathRate) - pexp((1/transitionRate_f)*(i - 1), rate = femaleDeathRate))/length(ind_f_wAlb_i)
    }
    else
    {
      immigrationRate_vector[ind_f_wAlb_i] <- overallFemaleImmigrationRate*(1 - pexp((1/transitionRate_f)*i, rate = femaleDeathRate))/length(ind_f_wAlb_i)
    }
  }
}
...
if(transitionRate_imm*I_bar/(femaleBirthRate*F_bar_mated) < 1)
{
...
#ID = seq(1,150,1)
#ts$ID <- ID
#plot(ts$ID, ts$block1_wAlbAB_m, xlab = "Days", ylab = "Population", main = "wAlbAB_m")
#plot(ts$ID, ts$block1_wAlbAB_f, xlab = "Days", ylab = "Population", main = "wAlbAB_f")
#plot(ts$ID, ts$block1_wAlbAB_imm, xlab = "Days", ylab = "Population", main = "wAlbAB_Imm")
#plot(ts$ID, ts$block1_wPip_m, xlab = "Days", ylab = "Population", main = "wPip_m")
#plot(ts$ID, ts$block1_wPip_f, xlab = "Days", ylab = "Population", main = "wPip_f")
#plot(ts$ID, ts$block1_wPip_imm, xlab = "Days", ylab = "Population", main = "wPip_imm")
