...
run_simulations <- function(OF, FCP, female_immigration_rate, male_immigration_rate, 
  ...
  if(!dir.exists(here::here(glue::glue("outputs/intervention_simulations/{parameter_set}/scenario_{scenario}")))){
    dir.create(here::here(glue::glue("outputs/intervention_simulations/{parameter_set}/scenario_{scenario}")))
  }
  ...
  res <- parallel::mclapply(
    ...
    function(j){
      if(all(transitionRate_imm*I_bar/(femaleBirthRate*F_bar_mated) < 1)) # Make sure the parameters are biologically feasible
      {
        ...
        file_name <- glue::glue("outputs/intervention_simulations/{parameter_set}/scenario_{scenario}/outputs-run{j}.csv")
