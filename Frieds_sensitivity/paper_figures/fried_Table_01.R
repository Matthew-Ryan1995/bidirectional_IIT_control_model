...
param_combs <- c("low", "mid")
# param_combs <- "mid"
for(pp in unique(param_combs)){
  
  dat_full <- read_rds(here::here(glue::glue("data/intervention_results_Fried_{pp}.Rds")))
  ...
  latex_code <- paste0("\\begin{table}[!h]
      \\caption{Percentage of simulations with management success for \\walbab\\ and \\wpip\\ populations under different scenarios (with the ", pp, " of Fried's Index) for immigration rates and intervention strategies. Management success is defined as keeping the \\walbab\\ population below $10\\%$ of the initial population size, and \\wpip\\ below $40\\%$ (unstable equilibrium threshold). The data is shown both within the stopping time and six months after. \\textbf{The model parameters are the expected values defined in Table \\ref{tab:parameters}.}}
