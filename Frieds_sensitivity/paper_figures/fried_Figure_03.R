...
for(pp in unique(param_combs)){
  ...
  plot_data_medians <- plot_data %>%
    ...
    ggdist::curve_interval(.exclude=c("run", "controlProp", "params", "fried"), 
                           .width = width) %>% 
    ungroup()
  ...
  p_storm_all <- 
    # ggplot(data = plot_data, aes(x=days)) +
  ...
  # # p_storm
  ggsave(here::here(glue::glue("img/fried_intervention_stormclouds_{pp}.png")),
         plot = p_storm,
