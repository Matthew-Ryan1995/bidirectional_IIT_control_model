## Matt Ryan
## 12/01/2026
## Run all scenario with variation in Fried's index.
...
num_runs <- 1#500
start_time <- 0
end_time <- 10#2*365 # MM: do we also need to edit this and add extra 180 days?
mc.cores <- 1# parallel::detectCores() - 1 # CHANGE WHEN ON BOWEN
...
if(GENERATE_MODEL_PARAMETERS){ # fixme: Due to the min/max flags and transformation flags, running this may not align with what you want to do.
  source(here::here("code/create_model_parameters_Fried_sensitivity.R"))
}
...
if(!dir.exists(here::here(glue::glue("outputs/Fried/intervention_simulations/{param_list$parameter_set}")))){
  dir.create(here::here(glue::glue("outputs/Fried/intervention_simulations/{param_list$parameter_set}")), recursive=TRUE)
}
